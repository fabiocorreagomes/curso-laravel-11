...
@if (session()->has('message'))
    <div class="p-4 mb-4 text-sm text-blue-800 rounded-lg bg-blue-50" role="alert">{{ session('message') }}</div>
@endif
...
@if ($errors->any())
        <ul>
            @foreach ($errors->all() as $error)
                <li class="p-4 mb-4 text-sm text-yellow-800 rounded-lg">{{ $error }}</li>
            @endforeach
        </ul>
@endif